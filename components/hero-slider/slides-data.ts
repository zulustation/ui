export const slidesData = [
  {
    id: 1,
    title: {
      text: "Starship\nIn Outer Space?",
      secondary: "WIN 1000 ZUL",
      styles:
        "text-white font-sans whitespace-pre font-extrabold text-6xl md:text-8xl mb-4",
      stylesSecondary:
        "text-center sm:text-start text-white font-sans whitespace-pre font-extrabold text-3xl md:text-4xl mb-6",
    },
    bg: "/carousel/spaceship_banner_ai.png",
    cta1: "Trade Now",
    link1: "https://app.zulu.pm/markets/125",
    color1: {
      primary: "#3679E1",
      secondary: "#FFF",
      border: "#3679E1",
    },
    cta2: "",
    link2: "",
    color2: {
      primary: "",
      secondary: "",
      border: "",
    },
  },
  {
    id: 2,
    title: {
      text: `Zulu\nis moving to Polkadot`,
      styles:
        "md:max-w-[660px] text-white font-sans whitespace-pre font-semibold text-5xl md:text-7xl mb-6 ",
    },
    bg: "/carousel/slide-1.jpeg",
    cta1: "Read More",
    link1:
      "https://blog.zulu.pm/zulu-will-migrate-to-the-polkadot-relay-chain/",
    color1: {
      primary: "#E90303",
      secondary: "#FFF",
      border: "#E90303",
    },
    cta2: "",
    link2: "https://blog.zulu.pm/zulu-newsletter-002/",
    color2: {
      primary: "transparent",
      secondary: "#FFF",
      border: "#FFF",
    },
  },

  // {
  //   id: 3,
  //   title: {
  //     text: "ZUL is now traded on DEXs",
  //     styles: "text-white text-3xl sm:text-4xl md:text-6xl",
  //   },
  //   bg: "/carousel/slide-3.jpeg",
  //   cta1: "Visit In-App Exchange",
  //   link1: "https://shapethefuture.zulu.pm",
  //   color1: {
  //     primary: "#F7FF58",
  //     secondary: "#000",
  //     border: "#F7FF58",
  //   },
  //   cta2: "",
  //   link2: "https://shapethefuture.zulu.pm",
  //   color2: {
  //     primary: "",
  //     secondary: "",
  //     border: "",
  //   },
  // },
  // {
  //   id: 4,
  //   title: {
  //     text: `Presenting Zulu’s
  //     "Rikiddo Scoring Rule`,
  //     styles: "text-white text-2xl sm:text-3xl md:text-5xl",
  //   },
  //   bg: "/carousel/slide-4.jpeg",
  //   cta1: "New Markets",
  //   link1: "https://shapethefuture.zulu.pm",
  //   color1: {
  //     primary: "#F7FF58",
  //     secondary: "#000",
  //     border: "#F7FF58",
  //   },
  //   cta2: "",
  //   link2: "https://shapethefuture.zulu.pm",
  //   color2: {
  //     primary: "",
  //     secondary: "",
  //     border: "",
  //   },
  // },
];
